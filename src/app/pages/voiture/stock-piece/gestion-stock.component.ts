...
export class GestionStockComponent {
  displayedColumns: string[] = ['piece', "marquePiece", "marqueVoiture", "modeleVoiture", "typeTransmission", "entree", "sortie", "prixUnitaire", "Date et heure", "Manager", 'actions'];
  stocks: GestionStock[];
  ...
  async addNewStockAsync(): Promise<GestionStock | undefined> {
    if (this.newStock) {
      console.log(this.newStock);
      try {
        const gestionStock = await firstValueFrom(this.gestionStockService.addGestionStock(this.newStock));
  ...
  async openModal(errorMessage: string = '') {
    const data = {
      ...
      fields: [
        {
          name: 'id_piece', label: 'Piece', type: 'select', required: true, defaultValue: this.newStock.id_service,
          options: this.pieces
        },
        ...
        { name: 'quantite', label: 'Quantité', type: 'text', required: true, defaultValue: this.newStock.quantite },
      ],
      submitText: 'Ajouter',
    ...
    dialogRef.afterClosed().subscribe(async result => {
      if (result) {
        try {
          ...
          this.newStock = result.libelle;
          // await this.addNewStockAsync();
        } catch (error: any) {
